...
public:
    ...
    static void getIndexDiffs(const ChessInterface& ciSource,
        const ChessInterface& ciTarget, Color c, IndexArray& added,
        IndexArray& removed, size_t& added_count, size_t& removed_count)
    {
        ...
        while (const_cast<const ChessInterface&>(ci) != ciSource) {
            if (c == nnue::White)
                getChangedIndices<nnue::White>(
                    ci, added, removed, added_count, removed_count);
            else
                getChangedIndices<nnue::Black>(
                    ci, added, removed, added_count, removed_count);
            if (!ci.previous())
                break;
        }
    ...
    static void updateAccumIncremental(const Network& network,
        const ChessInterface& ciSource, ChessInterface& ciTarget, const Color c)
    {
        IndexArray added, removed;
        size_t added_count, removed_count;
        getIndexDiffs(
            ciSource, ciTarget, c, added, removed, added_count, removed_count);
        /**
        std::cout << "incr " << (c == nnue::White ? "White" : "Black") << ":";
        std::cout << " removed ";
        for (unsigned i = 0; i < removed_count; i++) std::cout <<
        int(removed[i]) << ' '; std::cout << " added "; for (unsigned i = 0; i <
        added_count; i++) std::cout << int(added[i]) << ' '; std::cout <<
        std::endl;
        **/
        ...
        ciTarget.getAccumulator().setState(
            targetHalf, AccumulatorState::Computed);
    }

    // Full evaluation of 1/2 of the accumulator for a specified color (c)
    static void updateAccum(const Network& network, const IndexArray& indices,
        Color c, Color sideToMove, Network::AccumulatorType& accum)
    {
        auto it = network.layers.begin();
        AccumulatorHalf targetHalf
    ...
    static void updateAccum(
        const Network& network, ChessInterface& intf, const Color c)
    {
        ...
        if (incrementalOk
            && ci.getAccumulator().getState(half)
    ...
    static Network::OutputType fullEvaluate(
        const Network& network, ChessInterface& intf)
    {
...
private:
        ...
        Network::AccumulatorType& accum)
    {
        ...
        for (Color color : colors) {
            ...
            reinterpret_cast<Network::Layer1*>(*(network.layers.begin()))
                ->updateAccum(indices, targetHalf, accum);
            accum.setState(targetHalf, AccumulatorState::Computed);
        }
    }
};
