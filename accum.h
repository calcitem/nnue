...
public:
...
#ifdef SIMD
        if constexpr (size * 16 % simd::simdWidth == 0
            && sizeof(WeightType) == sizeof(OutputType)
            && sizeof(WeightType) == 2) {
...
#ifdef SIMD
        if constexpr ((size / 2) * 16 % simd::simdWidth == 0
            && sizeof(WeightType) == sizeof(OutputType)
            && sizeof(WeightType) == 2) {
    ...
    void copy_half(AccumulatorHalf half,
        const Accumulator<OutputType, WeightType, BiasType, size, alignment>&
            source,
