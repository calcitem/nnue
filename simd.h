...
template <typename T> static inline size_t chunks(unsigned len)
{
...
template <size_t inputSize, size_t outputSize> inline void dotProductnx32(
    const uint8_t* input, const int8_t weights[outputSize][inputSize],
    const int32_t* biases, int32_t* output)
{
#ifdef AVX2
    ...
    for (unsigned i = 0; i < outputSize; i++) {
        ...
        for (unsigned j = 0; j < chunks<uint8_t>(inputSize) / 2; ++j) {
            vec_t p0 = _mm_madd_epi16(
                _mm_maddubs_epi16(inp[2 * j + 0], row[2 * j + 0]), ones);
            vec_t p1 = _mm_madd_epi16(
                _mm_maddubs_epi16(inp[2 * j + 1], row[2 * j + 1]), ones);
            vec_t sum = _mm_add_epi32(p0, p1);
            sum = _mm_add_epi32(sum, _mm_shuffle_epi32(sum, 0xb));
            total = _mm_add_epi32(total, sum);
        }
#ifdef SSE41
        output[i] = _mm_cvtsi128_si32(total) + _mm_extract_epi32(total, 1)
            + biases[i];
#else
    ...
    for (unsigned i = 0; i < outputSize; i++) {
        ...
        for (unsigned j = 0; j < chunks<uint8_t>(inputSize); ++j) {
            ...
            __m128i extended_input_hi = _mm_unpackhi_epi8(input_j, zeros);
            __m128i product_lo
                = _mm_madd_epi16(extended_row_lo, extended_input_lo);
            __m128i product_hi
                = _mm_madd_epi16(extended_row_hi, extended_input_hi);
            sum_lo = _mm_add_epi32(sum_lo, product_lo);
            sum_hi = _mm_add_epi32(sum_hi, product_hi);
        }
        __m128i sum = _mm_add_epi32(sum_lo, sum_hi);
        __m128i sum_high_64 = _mm_shuffle_epi32(sum, _MM_SHUFFLE(1, 0, 3, 2));
        sum = _mm_add_epi32(sum, sum_high_64);
        __m128i sum_second_32
            = _mm_shufflelo_epi16(sum, _MM_SHUFFLE(1, 0, 3, 2));
        sum = _mm_add_epi32(sum, sum_second_32);
...
template <size_t size, typename InType, typename OutType> inline void clamp(
    const InType* in, OutType* out, [[maybe_unused]] InType clampMax)
{
    const vec_t* inp = reinterpret_cast<const vec_t*>(in);
    vec_t* outp = reinterpret_cast<vec_t*>(out);
#ifdef AVX2
    ...
    for (size_t i = 0; i < chunks<OutType>(size); ++i) {
        __m128i out0, out1;
        __m128i words0
            = _mm_load_si128(reinterpret_cast<const __m128i*>(inp + 2 * i + 0));
        __m128i words1
            = _mm_load_si128(reinterpret_cast<const __m128i*>(inp + 2 * i + 1));
...
template <size_t size, typename InType, typename OutType>
inline void scale_and_clamp(const InType* in, OutType* out, unsigned rshift,
    [[maybe_unused]] InType clampMax)
{
    const vec_t* inp = reinterpret_cast<const vec_t*>(in);
    vec_t* outp = reinterpret_cast<vec_t*>(out);
#ifdef AVX2
    ...
    const __m256i zero = _mm256_setzero_si256();
    for (size_t i = 0; i < chunks<OutType>(size); ++i) {
        // load 2x256 bit registers of shifted input data (32 bit input, 16 bit
        // output)
        __m256i r1 = _mm256_srai_epi16(
            _mm256_packs_epi32(inp[4 * i + 0], inp[4 * i + 1]), rshift);
        __m256i r2 = _mm256_srai_epi16(
            _mm256_packs_epi32(inp[4 * i + 2], inp[4 * i + 3]), rshift);
        // clamp and store into one 256-bit output chunk
...
#endif
    for (size_t i = 0; i < chunks<OutType>(size); ++i) {
        // load 2x128 bit registers of shifted input data (32 bit input, 16 bit
        // output) and clamp
        vec_t r1 = _mm_srai_epi16(
            _mm_packs_epi32(inp[4 * i + 0], inp[4 * i + 1]), rshift);
        vec_t r2 = _mm_srai_epi16(
            _mm_packs_epi32(inp[4 * i + 2], inp[4 * i + 3]), rshift);
...
#else
            _mm_subs_epi8(
                _mm_adds_epi8(_mm_packs_epi16(r1, r2), k0x80s), k0x80s);
#endif
    }
#endif
}
